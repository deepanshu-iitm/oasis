...
from director.ai import generate_ai_plan
...
def run_director(
    ...
    *,
    ...
    if use_ai:
        if on_step:
            on_step("openai_director")
        plan = generate_ai_plan(brief, duration_sec)
        if on_step:
            on_step("map_assets")
        plan.beats = map_assets(plan.beats, screenshot_paths)
    else:
        if on_step:
            on_step("parse_brief")
